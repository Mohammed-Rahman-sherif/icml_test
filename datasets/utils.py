...
class SimpleImagePatchDataset(TorchDataset):
    def __init__(self, data_source, patch_processor):
        self.data_source = data_source
        self.patch_processor = patch_processor
    ...
    def __getitem__(self, idx):
        ...
        # 1. Get 18 Crops (List of PIL Images)
        patches_pil = fixed_grid_crops(image, nodes=17)
        
        # 2. Convert to Tensor Stack [18, 3, 224, 224]
        # We assume patch_processor is the torchvision Compose transform
        patches_tensor = torch.stack([self.patch_processor(p) for p in patches_pil])

        return patches_tensor, item.label
