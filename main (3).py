...
def run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, image_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (batched_graphs, images, target) in enumerate(tqdm(train_loader_F)):
            ...
            graph_out, text_out = full_model(patches, text_features_for_nodes)
...
def main():
    ...
    model_id = cfg['backbone'][0]
    clip_model_vit = CLIPModel.from_pretrained(model_id).to(device)
    patch_processor = CLIPProcessor.from_pretrained(model_id)
...
#     # clip_model_resnet = clip_model_resnet.float()
    ...
    hgt_base = HGTImageFeatureExtractor(
    ...
    train_loader = torch.utils.data.DataLoader(
        SimpleImagePatchDataset(dataset.train_x, vit_preprocess),
